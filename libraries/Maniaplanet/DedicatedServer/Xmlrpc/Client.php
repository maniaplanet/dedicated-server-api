...
class Client 
{
	...
	function readCallbacks() 
	{
		...
		@stream_set_timeout($this->socket, 0, $this->timeout * 200);  // timeout 10 ms (to read available data)
		...
		try
		{
			$nb = @stream_select($read, $write, $except, 0, $this->timeout * 100); // select stream within 5ms
		}
		catch (\Exception $e)
		{
