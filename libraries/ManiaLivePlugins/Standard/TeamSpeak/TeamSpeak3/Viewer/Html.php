 ...
 *
 * $Id: Text.php 9/26/2011 7:06:29 scp@orilla $
 *
...
class Html implements ViewerInterface
{
   ...
   *
   * @var \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Node\AbstractNode
   */
  ...
  protected function getCorpusName()
  {
    ...
    if($this->currObj instanceof \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Node\Client)
    {
      ...
      foreach($this->currObj->memberOf() as $group)
      {
        ...
        elseif($group->getProperty("namemode") == \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\TeamSpeak3::GROUP_NAMEMODE_BEHIND)
        {
  ...
  protected function getSuffixIcon()
  {
    ...
    elseif($this->currObj instanceof \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Node\Channel)
    {
    ...
    if($this->currObj["virtualserver_icon_id"])
    {
      if(!$this->currObj->iconIsLocal("virtualserver_icon_id") && $this->ftclient)
      {
        if(!isset($this->cacheIcon[$this->currObj["virtualserver_icon_id"]]))
        {
          ...
          {
            $download = \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3::factory("filetransfer://" . $download["host"] . ":" . $download["port"])->download($download["ftkey"], $download["size"]);
          }
        ...
        if($this->ftclient == "data:image")
        {
          $html .= $this->getImage("data:" . \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Helper\Convert::imageMimeType($download) . ";base64," . base64_encode($download), "Server Icon", null, FALSE);
        }
        else
        {
  ...
  protected function getSuffixIconChannel()
  {
    ...
    if($this->currObj["channel_icon_id"])
    {
      if(!$this->currObj->iconIsLocal("channel_icon_id") && $this->ftclient)
      {
        ...
        if($this->ftclient == "data:image")
        {
          $html .= $this->getImage("data:" . \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Helper\Convert::imageMimeType($download) . ";base64," . base64_encode($download), "Channel Icon", null, FALSE);
        }
        else
        {
  ...
  protected function getSuffixIconClient()
  {
    ...
    foreach($this->currObj->memberOf() as $group)
    {
      ...
      $type = ($group instanceof \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Node\Servergroup) ? "Server Group" : "Channel Group";
      ...
      if(!$group->iconIsLocal("iconid") && $this->ftclient)
      {
        ...
        if($this->ftclient == "data:image")
        {
          $html .= $this->getImage("data:" . \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Helper\Convert::imageMimeType($download) . ";base64," . base64_encode($download), $group . " [" . $type . "]", null, FALSE);
        }
        else
        {
    ...
    if($this->currObj["client_icon_id"])
    {
      if(!$this->currObj->iconIsLocal("client_icon_id") && $this->ftclient)
      {
        ...
        if($this->ftclient == "data:image")
        {
          $html .= $this->getImage("data:" . \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Helper\Convert::imageMimeType($download) . ";base64," . base64_encode($download), "Client Icon", null, FALSE);
        }
        else
        {
