 ...
 *
 * $Id: ServerQuery.php 9/26/2011 7:06:29 scp@orilla $
 *
...
class ServerQuery extends AbstractAdapter
{
  ...
  {
    if($this->getTransport() instanceof \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Transport\AbstractTransport && $this->transport->isConnected())
    {
      try
      {
        $this->request("quit");
  ...
  public function wait()
  {
    ...
    } while($evt instanceof \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Helper\String && !$evt->section(\ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\TeamSpeak3::SEPARATOR_CELL)->startsWith(\ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\TeamSpeak3::EVENT));

    return new ServerQuery\Event($evt, $this->getHost());
  }
  
  public function readNotifications()
  {
	while($this->getTransport()->isReadyRead())
	{
		$evt = $this->getTransport()->readLine("\n", true);
		if($evt instanceof \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Helper\String && $evt->section(\ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\TeamSpeak3::SEPARATOR_CELL)->startsWith(\ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\TeamSpeak3::EVENT))
			new ServerQuery\Event($evt, $this->getHost());
	}
  }
  ...
  public function prepare($cmd, array $params = array())
  {
    ...
    foreach($params as $ident => $value)
    {
      ...
      else
      {
        ...
        elseif($value instanceof \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Node\AbstractNode) $value = $value->getId();

        $args[] = $ident . \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Helper\String::factory($value)->escape();//->toUtf8();
      }
    }

    foreach(array_keys($cells) as $ident) $cells[$ident] = implode(\ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\TeamSpeak3::SEPARATOR_CELL, $cells[$ident]);

    if(count($args)) $cmd .= " " . implode(\ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\TeamSpeak3::SEPARATOR_CELL, $args);
    if(count($cells)) $cmd .= " " . implode(\ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\TeamSpeak3::SEPARATOR_LIST, $cells);
   ...
   *
   * @return \ManiaLivePlugins\Standard\TeamSpeak\TeamSpeak3\Node\Host
   */
